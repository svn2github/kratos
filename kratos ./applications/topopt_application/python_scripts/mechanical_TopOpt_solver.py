    ...
    def Initialize(self):
        ...
        if(self.time_integration_method == "Implicit"):
          ...
          if(self.component_wise):
              print("component_wise Mechanical Solver not defined in TopOpt Application")
          else:
              if(self.line_search):
                  print("line_search Mechanical Solver not defined in TopOpt Application")
              else:
                  ...
                  self.mechanical_adjoint_solver = AdjointStructureStrategy(self.model_part, self.linear_solver, self.domain_size)
...
#######################################################################################################    
    def ComputeSensitivities(self):
        (self.mechanical_adjoint_solver).ComputeSensitivities(self.model_part)
#######################################################################################################
    def FilterSensitivities(self):
        (self.mechanical_adjoint_solver).FilterSensitivities(self.model_part)
#######################################################################################################
    def PrepareFilter(self):
        (self.mechanical_adjoint_solver).PrepareFilter(self.model_part)
#######################################################################################################
    def UpdateDensities(self, volfrac, q): # Optimality Criteria Method
        (self.mechanical_adjoint_solver).UpdateDensities(self.model_part, volfrac, q)
#######################################################################################################
...
def CreateSolver(model_part, config):
    ...
    structural_solver = MechanicalTopOptSolver(model_part, config.domain_size)
    ...
    if(hasattr(config, "linear_solver_config")):
        print("LINEAR SOLVER TYPE: ", config.linear_solver_config.solver_type)
