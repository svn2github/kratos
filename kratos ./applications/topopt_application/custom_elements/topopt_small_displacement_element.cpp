 ...
 */
KRATOS_CREATE_LOCAL_FLAG( TopOptSmallDisplacementElement, COMPUTE_RHS_VECTOR,                 0 );
KRATOS_CREATE_LOCAL_FLAG( TopOptSmallDisplacementElement, COMPUTE_LHS_MATRIX,                 1 );
KRATOS_CREATE_LOCAL_FLAG( TopOptSmallDisplacementElement, COMPUTE_RHS_VECTOR_WITH_COMPONENTS, 2 );
KRATOS_CREATE_LOCAL_FLAG( TopOptSmallDisplacementElement, COMPUTE_LHS_MATRIX_WITH_COMPONENTS, 3 );

//******************************CONSTRUCTOR*******************************************
//************************************************************************************

TopOptSmallDisplacementElement::TopOptSmallDisplacementElement( IndexType NewId, GeometryType::Pointer pGeometry )
...
TopOptSmallDisplacementElement::TopOptSmallDisplacementElement( IndexType NewId, GeometryType::Pointer pGeometry, PropertiesType::Pointer pProperties )
...
TopOptSmallDisplacementElement::TopOptSmallDisplacementElement( TopOptSmallDisplacementElement const& rOther)
    :Element(rOther)
...
TopOptSmallDisplacementElement&  TopOptSmallDisplacementElement::operator=(TopOptSmallDisplacementElement const& rOther)
{
...
Element::Pointer TopOptSmallDisplacementElement::Create( IndexType NewId, NodesArrayType const& rThisNodes, PropertiesType::Pointer pProperties ) const
{
    return Element::Pointer( new TopOptSmallDisplacementElement( NewId, GetGeometry().Create( rThisNodes ), pProperties ) );
}
...
Element::Pointer TopOptSmallDisplacementElement::Clone( IndexType NewId, NodesArrayType const& rThisNodes ) const
{

    TopOptSmallDisplacementElement NewElement(NewId, GetGeometry().Create( rThisNodes ), pGetProperties() );
    ...
    return Element::Pointer( new TopOptSmallDisplacementElement(NewElement) );
}
...
TopOptSmallDisplacementElement::~TopOptSmallDisplacementElement()
{
}
...
TopOptSmallDisplacementElement::IntegrationMethod TopOptSmallDisplacementElement::GetIntegrationMethod() const
{
...
void TopOptSmallDisplacementElement::GetDofList( DofsVectorType& rElementalDofList, ProcessInfo& rCurrentProcessInfo )
{
...
void TopOptSmallDisplacementElement::EquationIdVector( EquationIdVectorType& rResult, ProcessInfo& rCurrentProcessInfo )
{
...
void TopOptSmallDisplacementElement::GetValuesVector( Vector& rValues, int Step )
{
    const unsigned int number_of_nodes = GetGeometry().size();
    const unsigned int dimension       = GetGeometry().WorkingSpaceDimension();
    unsigned int       element_size    = number_of_nodes * dimension;
...
void TopOptSmallDisplacementElement::GetFirstDerivativesVector( Vector& rValues, int Step )
{
    const unsigned int number_of_nodes = GetGeometry().size();
    const unsigned int dimension       = GetGeometry().WorkingSpaceDimension();
    unsigned int       element_size    = number_of_nodes * dimension;
...
void TopOptSmallDisplacementElement::GetSecondDerivativesVector( Vector& rValues, int Step )
{
    const unsigned int number_of_nodes = GetGeometry().size();
    const unsigned int dimension       = GetGeometry().WorkingSpaceDimension();
    unsigned int       element_size    = number_of_nodes * dimension;
...
void TopOptSmallDisplacementElement::SetValueOnIntegrationPoints( const Variable<double>& rVariable,
...
void TopOptSmallDisplacementElement::SetValueOnIntegrationPoints( const Variable<Vector>& rVariable,
...
void TopOptSmallDisplacementElement::SetValueOnIntegrationPoints( const Variable<Matrix>& rVariable,
...
void TopOptSmallDisplacementElement::SetValueOnIntegrationPoints( const Variable<ConstitutiveLaw::Pointer>& rVariable,
...
void TopOptSmallDisplacementElement::GetValueOnIntegrationPoints( const Variable<double>& rVariable,
        std::vector<double>& rValues,
        const ProcessInfo& rCurrentProcessInfo )
{
...
void TopOptSmallDisplacementElement::GetValueOnIntegrationPoints( const Variable<Vector>& rVariable,
        std::vector<Vector>& rValues,
        const ProcessInfo& rCurrentProcessInfo )
{
...
void TopOptSmallDisplacementElement::GetValueOnIntegrationPoints( const Variable<Matrix>& rVariable,
        std::vector<Matrix>& rValues,
        const ProcessInfo& rCurrentProcessInfo )
{
...
void TopOptSmallDisplacementElement::GetValueOnIntegrationPoints( const Variable<ConstitutiveLaw::Pointer>& rVariable,
        std::vector<ConstitutiveLaw::Pointer>& rValues,
        const ProcessInfo& rCurrentProcessInfo )
{
...
void TopOptSmallDisplacementElement::Initialize()
{
    KRATOS_TRY
...
void TopOptSmallDisplacementElement::SetGeneralVariables(GeneralVariables& rVariables,
...
void TopOptSmallDisplacementElement::InitializeGeneralVariables (GeneralVariables & rVariables, const ProcessInfo& rCurrentProcessInfo)
{
...
void TopOptSmallDisplacementElement::InitializeSystemMatrices(MatrixType& rLeftHandSideMatrix,
...

{
    ...
    if ( rCalculationFlags.Is(TopOptSmallDisplacementElement::COMPUTE_LHS_MATRIX) ) //calculation of the matrix is required
    {
    ...
    if ( rCalculationFlags.Is(TopOptSmallDisplacementElement::COMPUTE_RHS_VECTOR) ) //calculation of the matrix is required
    {
...
void TopOptSmallDisplacementElement::CalculateElementalSystem( LocalSystemComponents& rLocalSystem,
							 ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY

    // ---------------CHANGES FOR TOPOPT_APPLICATION---------------
    double E_min    = this->GetValue(E_MIN);
    double penalty  = this->GetValue(PENAL);
    double X        = this->GetValue(X_PHYS);

    // Update Youngs Modulus of each element for SIMP
    // The variable YOUNG_MODULUS is the original value (E0)
    double YoungModulus = E_min + pow(X,penalty) * ( this->GetValue(YOUNG_MODULUS) - E_min);
    this->GetProperties().SetValue(YOUNG_MODULUS,YoungModulus);
    // ------------Endof CHANGES FOR TOPOPT_APPLICATION-------------
    ...
    for ( unsigned int PointNumber = 0; PointNumber < integration_points.size(); PointNumber++ )
    {
        ...
        if ( rLocalSystem.CalculationFlags.Is(TopOptSmallDisplacementElement::COMPUTE_LHS_MATRIX) ) //calculation of the matrix is required
        {
        ...
        if ( rLocalSystem.CalculationFlags.Is(TopOptSmallDisplacementElement::COMPUTE_RHS_VECTOR) ) //calculation of the vector is required
        {
...
void TopOptSmallDisplacementElement::CalculateAndAddLHS(LocalSystemComponents& rLocalSystem, GeneralVariables& rVariables, double& rIntegrationWeight)
{

  //contributions of the stiffness matrix calculated on the reference configuration
  if( rLocalSystem.CalculationFlags.Is( TopOptSmallDisplacementElement::COMPUTE_LHS_MATRIX_WITH_COMPONENTS ) )
    {
...
void TopOptSmallDisplacementElement::CalculateAndAddRHS(LocalSystemComponents& rLocalSystem, GeneralVariables& rVariables, Vector& rVolumeForce, double& rIntegrationWeight)
{
    //contribution of the internal and external forces
    if( rLocalSystem.CalculationFlags.Is( TopOptSmallDisplacementElement::COMPUTE_RHS_VECTOR_WITH_COMPONENTS ) )
    {
...
double& TopOptSmallDisplacementElement::CalculateIntegrationWeight(double& rIntegrationWeight)
{
    const unsigned int dimension = GetGeometry().WorkingSpaceDimension();
...
void TopOptSmallDisplacementElement::CalculateRightHandSide( VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo )
{
    //create local system components
    LocalSystemComponents LocalSystem;

    //calculation flags
    LocalSystem.CalculationFlags.Set(TopOptSmallDisplacementElement::COMPUTE_RHS_VECTOR);
...
void TopOptSmallDisplacementElement::CalculateRightHandSide( std::vector< VectorType >& rRightHandSideVectors, const std::vector< Variable< VectorType > >& rRHSVariables, ProcessInfo& rCurrentProcessInfo )
{
    //create local system components
    LocalSystemComponents LocalSystem;
    ...
    //calculation flags
    LocalSystem.CalculationFlags.Set(TopOptSmallDisplacementElement::COMPUTE_RHS_VECTOR);
    LocalSystem.CalculationFlags.Set(TopOptSmallDisplacementElement::COMPUTE_RHS_VECTOR_WITH_COMPONENTS);
...
void TopOptSmallDisplacementElement::CalculateLeftHandSide( MatrixType& rLeftHandSideMatrix, ProcessInfo& rCurrentProcessInfo )
{
    //create local system components
    LocalSystemComponents LocalSystem;

    //calculation flags
    LocalSystem.CalculationFlags.Set(TopOptSmallDisplacementElement::COMPUTE_LHS_MATRIX);
...
void TopOptSmallDisplacementElement::CalculateLocalSystem( MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo )
{
    //create local system components
    LocalSystemComponents LocalSystem;
...
void TopOptSmallDisplacementElement::CalculateLocalSystem( std::vector< MatrixType >& rLeftHandSideMatrices,
						     ...
						     ProcessInfo& rCurrentProcessInfo )
{
    ...
    //calculation flags
    LocalSystem.CalculationFlags.Set(TopOptSmallDisplacementElement::COMPUTE_LHS_MATRIX_WITH_COMPONENTS);
    LocalSystem.CalculationFlags.Set(TopOptSmallDisplacementElement::COMPUTE_RHS_VECTOR_WITH_COMPONENTS);
    ...
    LocalSystem.CalculationFlags.Set(TopOptSmallDisplacementElement::COMPUTE_RHS_VECTOR);
    LocalSystem.CalculationFlags.Set(TopOptSmallDisplacementElement::COMPUTE_LHS_MATRIX,false);
    ...
    LocalSystem.CalculationFlags.Set(TopOptSmallDisplacementElement::COMPUTE_LHS_MATRIX,true);
...
void TopOptSmallDisplacementElement::InitializeSolutionStep( ProcessInfo& rCurrentProcessInfo )
{
    ClearNodalForces();
...
////************************************************************************************
////************************************************************************************
void TopOptSmallDisplacementElement::InitializeNonLinearIteration( ProcessInfo& rCurrentProcessInfo )
{
    ClearNodalForces();
}
...
void TopOptSmallDisplacementElement::FinalizeNonLinearIteration( ProcessInfo& rCurrentProcessInfo )
{
...
void TopOptSmallDisplacementElement::FinalizeSolutionStep( ProcessInfo& rCurrentProcessInfo )
{
    //create and initialize element variables:
    GeneralVariables Variables;
    this->InitializeGeneralVariables(Variables,rCurrentProcessInfo);
...
void TopOptSmallDisplacementElement::InitializeMaterial()
{
    KRATOS_TRY
...
void TopOptSmallDisplacementElement::ResetConstitutiveLaw()
{
    KRATOS_TRY
...
void TopOptSmallDisplacementElement::CalculateAndAddExternalForces(VectorType& rRightHandSideVector,
...
void TopOptSmallDisplacementElement::CalculateAndAddInternalForces(VectorType& rRightHandSideVector,
        GeneralVariables& rVariables,
        double& rIntegrationWeight)
{
    KRATOS_TRY
...
void TopOptSmallDisplacementElement::CalculateAndAddKuum(MatrixType& rLeftHandSideMatrix,
        GeneralVariables& rVariables,
        double& rIntegrationWeight
                                                  )
{
...
void TopOptSmallDisplacementElement::ClearNodalForces()
{
    KRATOS_TRY
...
void TopOptSmallDisplacementElement::AddExplicitContribution(const VectorType& rRHSVector,
...
void TopOptSmallDisplacementElement::CalculateKinematics(GeneralVariables& rVariables,
...
Matrix& TopOptSmallDisplacementElement::CalculateDeltaPosition(Matrix & rDeltaPosition)
{
    KRATOS_TRY
...
void TopOptSmallDisplacementElement::CalculateDisplacementGradient(Matrix& rH,
        const Matrix& rDN_DX)
{
    KRATOS_TRY
...
void TopOptSmallDisplacementElement::CalculateInfinitesimalStrain(const Matrix& rH,
...
void TopOptSmallDisplacementElement::CalculateVelocityGradient(const Matrix& rDN_DX,
        Matrix& rDF )
{
    KRATOS_TRY
...
//************************************************************************************
//************************************************************************************
void TopOptSmallDisplacementElement::CalculateDeformationMatrix(Matrix& rB,
        const Matrix& rDN_DX)
{
    KRATOS_TRY
    const unsigned int number_of_nodes = GetGeometry().PointsNumber();
...
double& TopOptSmallDisplacementElement::CalculateTotalMass( double& rTotalMass )
{
    KRATOS_TRY
...
Vector& TopOptSmallDisplacementElement::CalculateVolumeForce( Vector& rVolumeForce, GeneralVariables& rVariables )
{
    KRATOS_TRY
...
void TopOptSmallDisplacementElement::CalculateMassMatrix( MatrixType& rMassMatrix, ProcessInfo& rCurrentProcessInfo )
{
    KRATOS_TRY
...
void TopOptSmallDisplacementElement::CalculateDampingMatrix( MatrixType& rDampingMatrix, ProcessInfo& rCurrentProcessInfo )
{
    KRATOS_TRY
...
void TopOptSmallDisplacementElement::Calculate(const Variable<double> &rVariable,
                                               double &rOutput,
                                               ProcessInfo &rCurrentProcessInfo)
{
    // ---------------TOPOPT_APPLICATION---------------

    if (rVariable == DCDX) // Sensitivity Variable DCDX
    {
        // ################## Get Element Stiffness Matrix ##################
        MatrixType Ke  = Matrix();
        this->CalculateLeftHandSide( Ke, rCurrentProcessInfo );

        // ########## Loop through nodes of elements and create ue ##########
        Element::GeometryType& rGeom = this->GetGeometry();
        unsigned int NumNodes = rGeom.PointsNumber(); //NumNodes=8
        array_1d<double,24> ue;
        ...
        // ########## Calculate trans(U)*Ke*U and set variable DCDX ##########
        const double& E_min        = this->GetValue(E_MIN);
        double penalty      = this->GetValue(PENAL);
        double X            = this->GetValue(X_PHYS);
        double youngmodulus = this->GetValue(YOUNG_MODULUS);

        double Ee = E_min + pow(X,penalty) * ( youngmodulus - E_min);
        MatrixType Ke0 = Ke/Ee;

        array_1d<double,24> intermediateVector = prod( trans(ue), Ke0 );
        double U_Ke0_U = inner_prod( intermediateVector, ue );

        double dcdx= (-penalty) * (youngmodulus-E_min) * pow(X,penalty-1) * U_Ke0_U;
        this->SetValue(DCDX,dcdx);

        // ####################### Objective Function #######################

        double obj_fcn = Ee * U_Ke0_U;
        this->SetValue(OBJ_FCN, obj_fcn);

        // ########################## Print Results ##########################
        //        int ElemID = this->Id();
        //        KRATOS_WATCH(ElemID);
        //        std::cout<<"U*Ke*U= "<<U_K_U<<std::endl;
        //        KRATOS_WATCH(this->GetValue(DCDX));
        //        KRATOS_WATCH(this->GetValue(OBJ_FCN));
...
void TopOptSmallDisplacementElement::CalculateOnIntegrationPoints( const Variable<double>& rVariable, std::vector<double>& rOutput, const ProcessInfo& rCurrentProcessInfo )
{
...
void TopOptSmallDisplacementElement::CalculateOnIntegrationPoints( const Variable<Vector>& rVariable, std::vector<Vector>& rOutput, const ProcessInfo& rCurrentProcessInfo )
{
...
void TopOptSmallDisplacementElement::CalculateOnIntegrationPoints( const Variable<Matrix >& rVariable, std::vector< Matrix >& rOutput, const ProcessInfo& rCurrentProcessInfo )
{
    KRATOS_TRY
...
void TopOptSmallDisplacementElement::DecimalCorrection(Vector& rVector)
{
    KRATOS_TRY
 ...
 */
int  TopOptSmallDisplacementElement::Check( const ProcessInfo& rCurrentProcessInfo )
{
    KRATOS_TRY
...
void TopOptSmallDisplacementElement::save( Serializer& rSerializer ) const
{
...
void TopOptSmallDisplacementElement::load( Serializer& rSerializer )
{
